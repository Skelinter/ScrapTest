...
#Returns list of all item's names
def getAvailableItems() -> list:
    pass

#Returns data structure of all item's attributes (ExtandedItemsList)
def getAvailableItemsExtanded() -> dict:
    pass

#Gets unchecked item's names list, returns checked item's names list
def checkCertainItems(CheckList: list) -> list:
    ...
    for itemContainer in soup.find_all('div', class_="items-container"):
        ...
        for item in itemContainer.find_all(recursive=False):
            # Checking if item's tag attribute considered in check list
            if item['data-title'] in CheckList:
                checkedItems.append(item['data-title'])
    return checkedItems
